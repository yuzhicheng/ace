...
public enum NdCodePattern {
    ...
    QB("QB|QB[0-9]{6}|QB[0-9]{9}|QB[0-9]{12}", 14,2 ,""){
        @Override
        void setUp(Map<Integer, Integer> map) {
            map.put(2, 6);
            map.put(8, 3);
    ...
    VT("VT|VT[0-9]{3}|VT[0-9]{6}|VT[0-9]{9}|VT[0-9]{12}|VT[0-9]{15}|VT[0-9]{18}|VT[0-9]{21}|VT[0-9]{24}|VT[0-9]{27}", 29,2 ,""){
        @Override
        void setUp(Map<Integer, Integer> map) {
            map.put(2, 3);
            map.put(5, 3);
    ...
    CF("CF|CF[0-9]{4}|CF[0-9]{7}|CF[0-9]{10}", 12,2 ,""){
        @Override
        void setUp(Map<Integer, Integer> map) {
            map.put(2, 4);
            map.put(6, 3);
     ...
     * 体素地形资源库
     */
    VX("VX|VX[0-9]{3}|VX[0-9]{6}|VX[0-9]{9}|VX[0-9]{12}", 14,2 ,""){
        @Override
        void setUp(Map<Integer, Integer> map) {
            map.put(2, 3);
            map.put(5, 3);
     ...
     * 体素地形成品库
     */
    VP("VP|VP[0-9]{3}|VP[0-9]{6}|VP[0-9]{9}|VP[0-9]{12}", 14,2 ,""){
        @Override
        void setUp(Map<Integer, Integer> map) {
            map.put(2, 3);
            map.put(5, 3);
    ...
    public String getPrefix(String parentNdCode) {
        ...
        if (prefixLength < getMinPrefixLength()) {
            // 抛出具体语义的信息
            throw new MessageException(HttpStatus.INTERNAL_SERVER_ERROR,ErrorMessageMapper.CheckNdCodeRegularFail.getCode(),getMessage());
        }
        ...
        if (extendDigit == 0) {
            throw new MessageException(HttpStatus.INTERNAL_SERVER_ERROR,ErrorMessageMapper.CheckNdCodeRegularFail.getCode(),"已经是最细粒度的编码（不再可自动扩展）");
        }
        return prefix;
    }
    ...
    public static void checkNdCodeRelation(String categoryNdCode, String parentNdCode, String currentNdCode) {
        if (StringUtils.isEmpty(categoryNdCode) || StringUtils.isEmpty(parentNdCode)
            ...
            throw new MessageException(HttpStatus.INTERNAL_SERVER_ERROR,ErrorMessageMapper.CheckNdCodeRegularFail.getCode(),"分类维度编码、维度数据的编码:nd_code全部都不能为空字符串");
        }
        // 分类维度是否已备案
        NdCodePattern pattern = fromString(categoryNdCode);
        if (pattern == null) {
            throw new MessageException(HttpStatus.INTERNAL_SERVER_ERROR,ErrorMessageMapper.CheckNdCodeRegularFail.getCode(),"分类维度nd_code="+categoryNdCode+" 还没有备案，有需要请与LC沟通");
        }
        if (!pattern.isValidNdCodePrefix(currentNdCode) || currentNdCode.length() != pattern.getLength() || !currentNdCode.substring(0, CATEGORY_LENGTH).equals(categoryNdCode)) {
            throw new MessageException(HttpStatus.INTERNAL_SERVER_ERROR,ErrorMessageMapper.CheckNdCodeRegularFail.getCode(),"nd_code="+currentNdCode+"编码不符合规范: " + pattern.getPattern());
        }
        ...
        if (parentNdCode.equals(CATEGORYDATA_TOP_NODE_PARENT)) {
            ...
            if (pattern.getMinPrefixLength() == CATEGORY_LENGTH) {
                if (currentPrefix.length() > CATEGORY_LENGTH + pattern.getExtendDigit(CATEGORY_LENGTH)) {
                    throw new MessageException(HttpStatus.INTERNAL_SERVER_ERROR,ErrorMessageMapper.CheckNdCodeRegularFail.getCode(),"nd_code="+currentNdCode+" 编码不符合规范: 层级不对");
                }
            } else {
            ...
            if (parentNdCode.length() != pattern.getLength()
                ...
                throw new MessageException(HttpStatus.INTERNAL_SERVER_ERROR,ErrorMessageMapper.CheckNdCodeRegularFail.getCode(),"nd_code="+parentNdCode+" 编码不符合规范: " + pattern.getPattern());
            }
            ...
            if (currentPrefix.length() > parentPrefix.length() + extendDigit) {
                // 层级不对
                throw new MessageException(HttpStatus.INTERNAL_SERVER_ERROR,ErrorMessageMapper.CheckNdCodeRegularFail.getCode(),"nd_code:"+currentNdCode+","+parentNdCode+" 编码不符合规范: 层级不对");
            }
            if (!currentPrefix.contains(parentPrefix) || parentPrefix.contains(currentPrefix)) {
                // 父子编码不一致
                throw new MessageException(HttpStatus.INTERNAL_SERVER_ERROR,ErrorMessageMapper.CheckNdCodeRegularFail.getCode(),"nd_code:"+currentNdCode+","+parentNdCode+" 编码不符合规范: 父子编码不一致");
            }
        }
    }
